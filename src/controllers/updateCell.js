...
export function luckysheetupdateCell(row_index1, col_index1, d, cover, isnotfocus) {
    ...
    let top = row_pre + container_offset.top + Store.infobarHeight + Store.custombarHeight + Store.toolbarHeight + Store.calculatebarHeight + Store.columnHeaderHeight - scrollTop - 2;
    if(luckysheetFreezen.freezenhorizontaldata != null && row_index1 <= luckysheetFreezen.freezenhorizontaldata[1]){
        top = row_pre + container_offset.top + Store.infobarHeight + Store.custombarHeight + Store.toolbarHeight + Store.calculatebarHeight + Store.columnHeaderHeight - 2;
    }
    ...
    let input_postition = {
        ...
        "max-height": winH + scrollTop - row_pre - 20 - 15 - Store.toolbarHeight - Store.infobarHeight - Store.custombarHeight - Store.calculatebarHeight - Store.sheetBarHeight - Store.statisticBarHeight, 
        "left": left, 
        "top": top, 
    }
    ...
    if (d[row_index] != null && d[row_index][col_index] != null) {
        ...
        if(htValue == "0"){//0 center, 1 left, 2 right
            input_postition = { 
                ...
                "top":  row_pre + container_offset.top + Store.infobarHeight + Store.toolbarHeight + Store.custombarHeight + Store.calculatebarHeight + Store.columnHeaderHeight - scrollTop - 2, 
            }
