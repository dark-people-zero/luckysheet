...
const server = {
    ...
    saveParam: function (type, index, value, params) {
	    ...
	    d.i = index;
	    d.v = value;
		d.socketid = _this.socketid;

		if(server.authorization && server.getToken() != null) d.tn = server.getToken();
    ...
    openWebSocket: function(){
        ...
        if('WebSocket' in window){
	        ...
	        _this.websocket.onopen = function(e) {
				...
				// Mencegah websocket mengirimkan pesan dalam waktu lama dan menyebabkan terputusnya koneksi
	        ...
	        _this.websocket.onmessage = async function(result){
				...
				let {message,id} = data;
				// 用户退出时，关闭协同编辑时其提示框
				//Saat pengguna logout, kotak prompt saat menutup pengeditan kolaboratif
				if(message === 'disconected') { // disconected
				...
				if(type == 0) { // jika sudah connect
	            ...
	            } else if(type == 2){ //更新数据 // perbaharui data
	            ...
	            } else if(type == 3){ //多人操作不同选区("t": "mv")（用不同颜色显示其他人所操作的选区）// Beberapa orang mengoperasikan pilihan berbeda ("t": "mv") (menampilkan pilihan yang dioperasikan oleh orang lain dalam warna berbeda)
	                let id = data.id;
					...
					if(flag) {
						Store.cooperativeEdit.changeCollaborationSize.forEach(val => {
							if(val.id == id) {
								console.log(item);
								val.v = item.v[0] || item.v.range[0] || item.range[0]
								val.i = index
							}
						})
					} else {
					...
					if(index == Store.currentSheetIndex){//发送消息者在当前页面 // Orang yang mengirim pesan ada di halaman saat ini
					...
					//其他客户端切换页面时
					// Ketika klien lain berpindah halaman
					Store.cooperativeEdit.checkoutData.forEach(item => {
	            ...
	            } else if(type == 4){ //批量指令更新 // Pembaruan instruksi batch
